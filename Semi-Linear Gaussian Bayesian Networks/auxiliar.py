...
def final_metrics(skel_true, skel_pred, thd=0):
    """
    Compute skeleton metrics + extended precision/recall including node-type errors (THD).

    Args:
        skel_true (nx.Graph): Ground truth skeleton.
        skel_pred (nx.Graph): Predicted skeleton.
        thd (int): Type Hamming Distance (node-type errors).
    ...
    n_nodes = len(skel_true.nodes())
    k_types = n_nodes - thd

    k_total = k + k_types

    recall_total = (
        k_total / (n_true + n_nodes)
        if (n_true + n_nodes) > 0 else 0.0
    )

    precision_total = (
        k_total / (n_pred + n_nodes)
        if (n_pred + n_nodes) > 0 else 0.0
    )
